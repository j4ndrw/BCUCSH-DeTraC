...
def train_feature_extractor(
    ...
    class_names, x, y = preprocess_images(initial_dataset_path, 224, 224, num_classes, framework = "torch")
    ...
    for class_name in class_names:
        extracted_features = extract_features(initial_dataset_path, class_name, 224, 224, net, framework = "torch")
        np.save(os.path.join(extracted_features_path, f"{class_name}.npy"), extract_features)

    compute_confusion_matrix(y_true = Y_test, y_pred = net.infer(X_test), framework = "torch", mode = "feature_extractor")